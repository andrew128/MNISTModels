...
def run_combinations(simple_model, complex_model, most_complex_model, x_data, y_data):
    ...
    proportions = np.arange(0, 1.0, 0.1)
...
def run_combined(simple_model, complex_model, most_complex_model, inputs, labels, conf_value, proportion):
    '''
    ...
    complex_indices = np.where((simple_highest_probs < conf_value) & (random_values < proportion), indices, None)
    ...
    most_complex_indices = np.where((simple_highest_probs < conf_value) & (random_values > proportion), indices, None)

...
def run_combinations(simple_model, complex_model, x_data, y_data):
    ...
    '''
    p_yy, p_yn, p_ny, p_nn, yn_sub, ny_sub, nn_sub  = run_combined(simple_model, complex_model, x_data, y_data)
    return p_yy, p_yn, p_ny, p_nn, yn_sub, ny_sub, nn_sub
...
def run_combined(simple_model, complex_model, inputs, labels):
    ...
    for i in range(simple_results.shape[0]):
        ...
        if simple_results[i] and complex_results[i]:
            ...
            yn += 1
            if simple_highest_probs[i] - second_highest > 0.2:
                yn_sub += 1
    ...
    return yy, yn, ny, nn, yn_sub, ny_sub, nn_sub
...
def main():
    ...
    p_yy, p_yn, p_ny, p_nn, yn_sub, ny_sub, nn_sub = run_combinations(l1_model, l2_model, x_test, y_test)
    ...
    p_yy, p_yn, p_ny, p_nn, yn_sub, ny_sub, nn_sub = run_combinations(l1_model, l3_model, x_test, y_test)
    ...
    p_yy, p_yn, p_ny, p_nn, yn_sub, ny_sub, nn_sub = run_combinations(l2_model, l3_model, x_test, y_test)

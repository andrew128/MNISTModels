...
def run_combinations(simple_model, complex_model, x_data, y_data):
    ...
    '''
    conf_values = np.arange(0.8, 1.01, 0.01)
    thresholds = np.arange(0.76, 1.0, 0.04)
...
def run_combined(simple_model, complex_model, inputs, labels, conf_value, threshold):
    '''
...
def main():
    ...
    for i in range(1):
        ...
        all_conf_values, accuracies, times, simple_percents, thresholds = run_combinations(l1_model, l2_model, x_test, y_test)
